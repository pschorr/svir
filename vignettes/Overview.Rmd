...
In addition to the percentile rank values for each of the three tiers, census tracts with values in the bottom and top 10% of each tier receive a “flag”. These flags serve as a means of identifying census tracts with particular vulnerabilities in specific socio-demographic areas, that might otherwise be masked by less vulnerable rankings in other socio-demographic variables.

In previous implementations of SVI, these relative values were calculated for each census tract at the national, and state levels. Blagh blagh blagh, adding more to this section now...
...
**1. Create Database and add SVI shapefile**

**2. Create UI for Shiny web application template**

**3. Create File Upload widget UI**

**4. Write File Upload server logic to accept shapefiles**

**5. Perform PostGIS geometry intersection using user input shapefile and census tracts**

6. Add leaflet map to UI 

7. Rerank and calculate SVI for census tracts in AOI

8. Calculate "flags" (if any) within AOI

9. Write server logic to plot census tracts and flags on leaflet map

10. Create Download widget UI

11. Write server logic to download SVI in AOI
...
1. November 20 - November 26


2. November 27 - December 3


3. December 4 - December 10


4. December 11 - December 17
...
![Census Tracts Selected by Hurricane Sandy Impact Shapefile)](../vignettes/RegionalSVI_ShinyGUI_11_20_17.png)
